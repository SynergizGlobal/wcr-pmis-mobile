...
class ListOfContractsReportScreen extends StatefulWidget {
  const ListOfContractsReportScreen({
    super.key,
    required this.args,
  ...
  @override
  State<ListOfContractsReportScreen> createState() =>
      _ListOfContractsReportScreenState();
}

class _ListOfContractsReportScreenState extends State<ListOfContractsReportScreen> {
  bool _loading = false;
  ...
  List<ContractDetailReportOption> _contractorOptions =
      <ContractDetailReportOption>[];
  List<ContractDetailReportOption> _contractStatusOptions =
      <ContractDetailReportOption>[];
  ...
  ContractDetailReportOption? _selectedContractStatus;
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    AppSelectSheetField<ContractDetailReportOption>(
                      ...
                      placeholderText: 'Select Contract Status',
                      items: _contractStatusOptions,
                      value: _selectedContractStatus,
                      enabled: !_loading && !_generating,
                      itemLabelBuilder: (ContractDetailReportOption option) =>
                          option.label,
                      onChanged: _onContractStatusChanged,
                    ),
                    const SizedBox(height: 14),
                    AppSelectSheetField<ContractDetailReportOption>(
    ...
    try {
      final List<List<Map<String, dynamic>>> responses =
        ...
        <Future<List<Map<String, dynamic>>>>[
          ...
          widget.dataSource.fetchListOfContractsReportContractStatusList(),
          widget.dataSource.fetchContractDetailReportStatusList(
            projectIdFk: _selectedProject?.value,
            contractorIdFk: _selectedContractor?.value,
            status: _selectedContractStatus?.value,
          ),
      ...
      setState(() {
        ...
        _contractStatusOptions = ensureContractDetailReportOptionInList(
          _selectedContractStatus,
          dedupeContractDetailReportOptions(
            responses[1].map(contractOpenStatusOptionFromRow),
          ),
        );
        ...
        _selectedContractStatus = keepContractDetailReportOption(
          _selectedContractStatus,
  ...
  Future<void> _onContractStatusChanged(
    ContractDetailReportOption contractStatus,
  ) async {
    setState(() {
  ...
  Future<void> _generateReport() async {
    ...
    try {
      final ({List<int> bytes, String? fileName}) result =
          await widget.dataSource.generateListOfContractsReport(
        projectIdFk: _selectedProject?.value,
        contractorIdFk: _selectedContractor?.value,
        contractStatus: _selectedContractStatus?.value,
        contractStatusFk: _selectedWorkStatus?.value,
      ...
      final String fileName = result.fileName?.trim().isNotEmpty == true
          ...
          : 'list_of_contracts_${DateTime.now().millisecondsSinceEpoch}.xlsx';
